...
async def choice_dress_front(request: Request):
    if user_choice.cursor.alive:
        ...
        user_choice.np_colors[image_info["id_image"]] = string_to_list(
            image_info["values"]
        )
        np_array_reshaped = np.array(
            user_choice.np_colors[image_info["id_image"]]
        ).reshape(404, 280, 3)
        img_str = image_to_str(np_array_reshaped)

        return templates.TemplateResponse(
            "dresschoicechoice.html",
            {
                "request": request,
                "images": img_str,
        ...
        best, worse = await user_choice.result_calc()
        return templates.TemplateResponse(
            "dresses_result.html",
            {"request": request, "best_images": best, "worse_images": worse},
        )
...
def add_dislike(request: Request, id_image: int):
