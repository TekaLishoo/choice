...
async def update_dresses():
    ...
    im = [
        Image.open(requests.get(img_lnk, stream=True).raw)
        for img_lnk in image_links_flatten[1::2]
    ]
    ...
    im_array = np.array(
        [np.asarray(img.resize((280, 404))).flatten() for img in im], dtype=int
    )
    ...
    succes_message = "Successfully updated mongo dresses table with %d objects." % int(
        dresses.count_documents({})
    )
